...
class TestCreateLoginToken:
    def test_login_token_has_correct_data(self, test_user_record: UserSchema):
    ...
    def test_decoding_login_token_fails_with_invalid_secret(
        self, test_user_record: UserSchema
    ):
        # generate login token for the user
        login_token = create_login_token(test_user_record)
        ...
        with pytest.raises(JWTError):
            jwt.decode(
                login_token, JWT_SECRET + "-invalid", algorithms=[ALGORITHM]
            )
